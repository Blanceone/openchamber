...
- `getVisibleContextRailSurfaces` is the single visibility filter shared by the
  ...
  drops the walkthrough on VS Code and below `WALKTHROUGH_MIN_WIDTH`, drops the
  wiki surface when `openWikiEnabled` is false, and hides `has-content`
  surfaces until a tab of their mode exists. Both consumers use it so the digit
  shown on a rail badge always maps to the same surface the shortcut opens.
