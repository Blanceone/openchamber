...
- `packages/web/server/lib/notifications/routes.js`: route registration for push, visibility, and session status/attention endpoints. APNs token routes return 410 in the local-desktop build.
...
- APNs native delivery runtime was removed; `createDisabledApnsRuntime()` in `local-desktop-remote.js` is wired at boot instead.
