...
import { assertOpenWikiModelGatewayReady, parseModelRef } from './model-bridge.js';
...
const DRAFT_MARKER_START = '<<<OPENCHAMBER_INSTRUCTIONS>>>';
const DRAFT_MARKER_MID = '<<<OPENCHAMBER_FORMAT>>>';
const DRAFT_MARKER_END = '<<<OPENCHAMBER_END>>>';
...
export const parseDraftMarkers = (text) => {
  ...
  const start = raw.indexOf(DRAFT_MARKER_START);
  const mid = raw.indexOf(DRAFT_MARKER_MID);
  const end = raw.indexOf(DRAFT_MARKER_END);
  if (start < 0 || mid < 0 || end < 0 || !(start < mid && mid < end)) {
    return null;
  }
  const instructions = raw.slice(start + DRAFT_MARKER_START.length, mid).trim();
  const format = raw.slice(mid + DRAFT_MARKER_MID.length, end).trim();
...
/**
 * @param {{ instructions: string, format: string }} draft
 * @param {{ instructions: string, format: string }} current
 */
const buildParsePrompt = (documents, current) => {
  ...
  return [
    'You help OpenChamber configure OpenWiki prompt files for a software repository wiki.',
...
export const mergeFormatDraft = async (input) => {
  ...
  const model = parseModelRef(input.openWikiModelOverride)
    || parseModelRef(input.model)
    || parseModelRef(draft.model);
  if (!model) {
    throw Object.assign(new Error('Model is required'), {
      statusCode: 400,
      code: 'model-required',
